...
const QUIZ = { 
  ...
  // inside literal array...
      //object within the array
      {
        ...
        sources: '<a href="https://en.wikipedia.org/wiki/Near_East">SOURCES: Near East</a>',
      ...
      },
      {
        ...
        sources: '<a href="https://en.wikipedia.org/wiki/Middle_East#Territories_and_regions">SOURCES: Middle East Territories and Regions</a>',
      ...
      },
      {
        ...
        sources: '<a href="https://en.wikipedia.org/wiki/Byzantine%E2%80%93Sasanian_War_of_602%E2%80%93628">SOURCES: Byzantine Sassanian War of 602-628</a>',
      ...
      },
      {
        ...
        sources: '<a href="https://en.wikipedia.org/wiki/Byzantine_Empire#Language">SOURCES: Byzantine Empire languages</a>',
      ...
      },
      {
        ...
        sources: '<ul><li><a href="https://en.wikipedia.org/wiki/Islamic_Golden_Age">SOURCE: Islamic Golden Age</a></li><li><a href="https://www.timemaps.com/civilizations/islamic-caliphate/">SOURCE: The Islamic Caliphate</a></li></ul>',
      ...
      },
      {
        ...
        sources: '<a href="https://en.wikipedia.org/wiki/Gunpowder_Empires">SOURCES: Wikipedia - Gun Powder Empires</a>',
      ...
      },
      {
        ...
        sources: '<a href="http://www.bbc.com/news/world-middle-east-25434060">SOURCES: World Middle East Population</a>',
      ...
      },
      {
        ...
        sources: '<ul><li><a href="https://en.wikipedia.org/wiki/Middle_East#Languages">SOURCE: Wikipedia - Middle East languages</a></li><li><a href="https://en.wikipedia.org/wiki/Semitic_languages">SOURCE: Semitic Languages</a></li><li><a href="https://en.wikipedia.org/wiki/Indo-Iranian_languages">SOURCE: Indo-Iranian(Aryan) languages</a></li><li><a href="https://en.wikipedia.org/wiki/Turkic_languages">SOURCE: Turkic Languages</a></li></ul>',
      ...
      },
      {
        ...
        sources: '<ul><li><a href="https://en.wikipedia.org/wiki/Persian_Jews">SOURCE: Persian Jews</a></li><li><a href="https://en.wikipedia.org/wiki/Iranian_Jews_in_Israel">SOURCE: Iranian Jews in Israel</a></li><li><a href="https://en.wikipedia.org/wiki/Cyrus_the_Great#cite_note-15">SOURCE: Cyrus The great as the mesiah</a></li></ul>',
...
var lastPercentageScore = '0.0';

// we want JQuery to generate html

function setupQuiz() {
  console.log('`startQuiz` ran');
  $('.js-start-button').on('click', startQuiz);
  //$('submit').on('click', checkResults);
}
...
function renderQuiz() {
  ...
  checkResults();
  //$('form').submit(checkResults(event));
}
...
//each switch should have a return
...
//might be best to have individual named functions for each compontent that needs to be generated

//we need a loop to call these functions every time a Question is responded to

// We'd want our data object to be a global and constant variable (best practice to keep these vars at the top after libraries, or above where it is first referenced). Suggested to use json for data object

//think about questions as an array, and a pointer letting us know which question we're at.
//you could loop through data.questions as an array
...
function thingHtml(thing) { 
  
  return 
}


// Note that node.js and JQuery both have the JSON library loaded by default as JSON

/*
// Victors example...
let thing = {
  something: "some thing here",
...
function thingHtml(thing){
  return `<div class="thing">
    <h2>${thing.title}</h2>
    <p><label>Something:</label> ${thing.something}</p>
  </div>
  `;
}

console.log(thingHtml(thing));
*/
